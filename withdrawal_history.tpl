{assign var="title" value="Withdraw History"}
...
<tr>
 ...
 <th class=inheader>Type</td>
 <th class=inheader>Credit</td>
 <th class=inheader>Debit</td>
 <th class=inheader>Balance</td>
 <th class=inheader>P.S.</td>
</tr>
{if $qtrans > 0}
...
{section name=trans loop=$trans}
<tr>
 <td class=item><b>{$trans[trans].transtype}</b></td>
 <td class=item width=200 align=right><b>{$currency_sign} {$trans[trans].actual_amount}</b> <img src="images/{$trans[trans].ec}.gif" align=absmiddle hspace=1 height=17> {if $trans[trans].transtype eq 'Withdrawal request'} <a href=?a=cancelwithdraw&id={$trans[trans].id} onclick="return confirm('Are you sure you want to delete this request?')">[cancel]</a>{/if}</td>
 <td class=item width=170 align=center valign=bottom><b><small>{$trans[trans].d}</small></b></td>
</tr>
<tr>
 <td class=item colspan=3 style="color: gray"><small>{$trans[trans].description}</small></td>
</tr>
{/section}
{else}
<tr>
